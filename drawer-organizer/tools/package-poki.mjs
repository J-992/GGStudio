 ...
 *
 * CI does not use this script -- `@poki/cli` makes its own archive. This is
 * the path for uploading by hand through the dashboard, and for seeing what
 * the pipeline would say before pushing.
 */
...
const ZIP = join(ROOT, 'drawer-organizer-poki.zip');
...
try
{
    //  `-X` drops the resource forks and finder metadata macOS otherwise
    //  buries in the archive; the paths are relative to dist/ so index.html
    //  lands at the root of the zip, which is where Poki looks for it.
