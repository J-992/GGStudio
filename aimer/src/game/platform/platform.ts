 ...
 *
 * The target is fixed at build time by `vite/config.shared.mjs`, because it has
 * to be: the portal hosts its own copy of the build. It is inlined as a bare
 * boolean so the bundler folds the branch below and drops the platform nobody
 * asked for -- a playtest build carries no Poki code and no Poki URL at all.
 *
...
const platform: GamePlatform = import.meta.env.VITE_POKI ? pokiPlatform : nonePlatform;
