...
{
    //  Poki's checklist is explicit that no SDK event may fire while an ad is
    //  on screen. The desired state is not lost -- it is held in
...
async function initialize (): Promise<boolean>
{
    try
    {
        ...
        //  A rejected init means an ad blocker, not a broken SDK. Poki asks
        //  games to carry on regardless, so the rejection is swallowed and the
        //  SDK is marked available: its methods stay callable and quietly no-op.
 ...
 * an upgrade screen, a pause or an ad. Calls are coalesced and the most recent
...
export function setPokiGameplayActive (active: boolean): void
{
    ...
    gameplaySync = (async () =>
    {
        ...
        {
            //  Cleared inline rather than from a `.finally` on the promise,
            //  which would run a tick later: a call landing inside that tick
            //  would find the guard still set, return early, and be lost by the
            //  loop that had already decided it was finished.
...
export async function startPokiLoading (): Promise<boolean>
{
    ...
    const s = sdk();
    if (loadingReported || !s?.gameLoadingStart) return false;

    try { s.gameLoadingStart(); loadingReported = true; return true; }
    catch { return false; }
}
...
export async function stopPokiLoading (): Promise<boolean>
{
    ...
    const s = sdk();
    if (!loadingReported || !s?.gameLoadingFinished) return false;

    try { s.gameLoadingFinished(); loadingReported = false; return true; }
    catch { return false; }
}
...
function runBreak (kind: 'commercial' | 'rewarded'): Promise<boolean>
{
    ...
    const attempt = (async () =>
    {
        ...
        try
        {
            //  Poki calls this back at the last moment before the ad renders,
            //  which is the only point at which muting is neither early nor late.
            const result = await call.call(s, () => publishAdMute(true));

            //  `commercialBreak` resolves with nothing; `rewardedBreak` resolves
            //  with whether the player earned the reward.
        ...
        finally
        {
            publishAdMute(false);
            adInFlight = false;
            //  Anything the game asked for mid-ad was held rather than sent.
            //  Replay the latest wanted state now that events are allowed
            //  again; the usual dedupe drops it if the state never moved.
...
/**
 * A rewarded video the player opted into. Resolves true only when the video
 * ran to completion and the reward is owed.
 */
...
export async function pokiHappyTime (intensity: number): Promise<boolean>
{
    ...
    const s = sdk();
    if (!s?.happyTime) return false;

    try { s.happyTime(clamped); return true; }
    catch { return false; }
}
...
export async function pokiCaptureError (error: unknown): Promise<boolean>
{
    ...
    const s = sdk();
    if (!s?.captureError) return false;

    try { s.captureError(error); return true; }
    catch { return false; }
}
