...
| Locked board slot (`slot_locked`) | `public/assets/slot-locked.webp` | Deterministic procedural pixel art from `tools/make_locked_tile.py`: the atlas's own `tile_stone` frame chilled, then an iron chain and padlock drawn pixel by pixel over it at the same 69×48 footprint | Original work, © Reaper8202 |
...
| Three mobile loading-screen key-art scenes | `art-source/loading-screen-01.png`–`loading-screen-03.png`; shipped as `public/assets/loading-splash-1.webp`–`loading-splash-3.webp` plus per-screen blurred placeholders inlined into `index.html` | AI-generated via OpenAI image generation for this project, composed natively for a 9:16 mobile display. `tools/make_splash.py` emits responsive WebP versions; the framed loading bar is rendered by the HTML shell. | Original project assets, © Reaper8202 |
