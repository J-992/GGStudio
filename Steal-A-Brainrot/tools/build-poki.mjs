 ...
 * globals through the window, and `index.html` is where their order is written
 * down. So the build reads that order out of the HTML rather than keeping a
 * second list here, concatenates the files in it, and rewrites the page to load
 * the one result. Add a script tag, and the build picks it up.
 *
 ...
 * scripts means `CREATURES` and `CFG` silently vanish from the output and the
 * game dies on Poki instead of here. The saving would be ~60 KB against a
 * 1.2 MB Phaser, which is not worth a build that can eat the creature data.
 *
 ...
 * Both are checked by `scripts/poki/preflight.mjs` at the repo root; `npm run
 * package` runs it. What this script does about them is drop the jsdelivr
 * fallback (fine when you serve the folder yourself, fatal in the sandbox) and
 * leave the SDK tag alone.
 */
...
//  A banner per file so a stack trace from Poki's CDN still names the source it
//  came from. `'use strict'` is deliberately absent: these are classic scripts
//  and adding it changes how they run.
...
if (!page.includes('game-cdn.poki.com')) die('the rewritten page has no Poki SDK tag; Poki rejects a build without one.');
...
//  assets/ is the character art plus the manifest that maps it onto texture
//  keys. Sound is still synthesised at runtime, so there is none of it here.
...
//  filter, and today nothing does. Depth first, so a directory holding only
//  empty directories goes too.
function prune(dir) {
